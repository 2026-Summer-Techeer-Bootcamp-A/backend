"""요구별 이력서 판정과 할루시네이션 가드, 가중 점수.

LLM이 met/partial로 판정하면서 이력서에 실제로 없는 문장을 인용하면 화면에
그대로 노출되지 않도록, 인용이 이력서 원문에 부분문자열로 존재하는지
...
_SYSTEM = (
    "너는 이력서가 공고 요구를 충족하는지 판정하는 분석기다. 요구마다 met(충족), "
    "partial(전이 가능한 인접 경험), gap(근거 없음) 중 하나를 매긴다. met과 partial은 "
    "이력서 원문에서 근거 문장을 반드시 그대로 인용한다. 지어내지 않는다. gap이면 한 문장으로 "
...
def judge_requirements(
    ...
    prompt = (
        ...
        '"resume_quote":"이력서 원문 인용","rationale":"판정 근거 한 줄",'
        '"next_step":"gap일 때 보완 제안"} 형식이다.'
    )
    out = llm.json(_SYSTEM, prompt, temperature=0.2, max_output_tokens=2048)
    ...
    if isinstance(items, list):
        for it in items:
            ...
            if verdict not in _VALID:
                verdict = "gap"
            quote = str(it.get("resume_quote") or "")
